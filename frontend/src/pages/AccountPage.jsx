...
const AccountPage = ({ pageTitle }) => {
  ...
  const handleAddAccount = (data, openModal, closeModal) => {
    ...
    setAccounts((prevAccounts) => {
      const newId = prevAccounts.length + 1 + Math.floor(Math.random() * 1000);
      const newAccount = {
        id: newId,
              ...
              <div className="ml-5 grid grid-cols-4 gap-4 mt-2">
                {accounts.map((account) => (
                  <div
                    ...
                    className={`cursor-pointer ${selectedAccount === account.id ? 'border-l-7 border-blue-900' : ''}`}
                  >
                ...
                <div className="bg-gray-300 text-white px-10 rounded-lg cursor-pointer">
                  <button
                    className="ml-6 text-gray-600 font-bold"
                    onClick={() =>
                      openModal(
                        <AddAccount
                          openModal={openModal}
