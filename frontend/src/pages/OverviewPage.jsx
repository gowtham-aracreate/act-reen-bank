...
const OverviewPage = ({ pageTitle }) => {
    ...
    const monthRange = ["This Month", "February", "January"];

    return (
        <Layout pageTitle={pageTitle}>
            <ModalLayout>
                {({ openModal, closeModal, setUserData }) => (
                    <div className='mt-0 flex items-top'>
                        {/* Overview Content */}
                        <div className='flex flex-grow gap-6'>
                            <div className='flex-1 w-1/2 pl-5 w-[380px]'>
                                ...
                                <div>
                                    <div className='flex'>
                                        ...
                                        <div className="pt-5 pl-14 flex items-center gap-3">
                                            {/* Eye Toggle */}
                                ...
                                <div className='pt-10'>
                                    <div className='flex items-center justify-between'>
                                        ...
                                        {/* Add Account Button */}
                                        <button
                                            className='bg-gray-200 w-8 h-8 flex items-center justify-center rounded-lg cursor-pointer'
                                            onClick={() => openModal(
                                                <AddAccount
                                                    openModal={openModal}
                                                    closeModal={closeModal}
                                                    setUserData={(data) => {
                                                        setAccounts(prev => [...prev, { title: data.accountName, amount: data.amount }]);
                                                        setUserData(data);
                                                    }}
                                                />
                                            )}
                                ...
                                <div className='pt-10'>
                                    <div className='flex gap-90'>
                                        ...
                                        <div className='flex items-center gap-1 bg-gray-200 px-1 rounded-lg cursor-pointer' onClick={() => setIsDropdownOpenMonth(!isDropdownOpenMonth)}>
                                            <span className='text-sm w-25 p-1'>{selectedMonth}</span>
                                            <img className='w-5 h-5' src={DropDown} alt="Drop Down Icon" />
                                        </div>

                                        {/* Dropdown for month */}
                                        {isDropdownOpenMonth && (
                                            <div className="absolute left-0 w-32 p-2 bg-white shadow-lg rounded-lg ml-187 mt-11">
                                                ...
                                                {monthRange.map((range, index) => (
                                                    <div
                                                        key={index}
                                                        className={`p-3 text-sm rounded-lg cursor-pointer ${selectedMonth === range ? "bg-gray-300" : "hover:bg-gray-200"}`}
                                                        onClick={() => {
                                    ...
                                    <div className='mt-2'>
                                        <div className="pb-6 flex items-center mt-1">
                                            ...
                                            <div className="ml-6 bg-green-500 h-3 rounded-sm" style={{ width: "40%" }}></div>
                                        ...
                                        <div className="flex items-center mt-1">
                                            ...
                                            <div className="ml-4 bg-red-400 h-3 rounded-sm" style={{ width: "30%" }}></div>
                            ...
                            {/* Transaction Content */}
                            <div className='w-1/2 p-2 min-w-[300px] pr-10'>
                                <Transactions />
                            </div>
                        </div>
                    </div>
